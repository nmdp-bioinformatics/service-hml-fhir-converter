...
@RequestMapping("/ssp")
@CrossOrigin
public class SspController implements SspApi {

    private final SspService sspService;
    private static final Logger LOG = Logger.getLogger(SspController.class);

    @Autowired
    public SspController(SspService sspService) {
    ...
    public Callable<ResponseEntity<Boolean>> deleteSsp(@RequestBody Ssp ssp) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(sspService.deleteItem(ssp), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<Boolean>> deleteSsp(@PathVariable String id) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(sspService.deleteItem(id), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<Ssp>> getModel() {
        try {
            ...
            return () -> new ResponseEntity<>(new Ssp(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
