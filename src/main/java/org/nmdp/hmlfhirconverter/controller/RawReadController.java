...
@RestController
@RequestMapping("/rawRead")
@CrossOrigin
public class RawReadController implements RawReadApi {

    private final RawReadService rawReadService;
    private static final Logger LOG = Logger.getLogger(RawReadController.class);

    @Autowired
    public RawReadController(RawReadService rawReadService) {
        this.rawReadService = rawReadService;
    }
    ...
    public Callable<ResponseEntity<Boolean>> deleteRawRead(@RequestBody RawRead rawRead) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(rawReadService.deleteItem(rawRead), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<Boolean>> deleteRawRead(@PathVariable String id) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(rawReadService.deleteItem(id), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<RawRead>> getModel() {
        try {
            ...
            return () -> new ResponseEntity<>(new RawRead(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
