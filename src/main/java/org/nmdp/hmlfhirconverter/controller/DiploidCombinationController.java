...
@RestController
@RequestMapping("/diploidCombination")
@CrossOrigin
public class DiploidCombinationController implements DiploidCombinationApi {

    private final DiploidCombinationService diploidCombinationService;
    private static final Logger LOG = Logger.getLogger(DiploidCombinationController.class);

    @Autowired
    public DiploidCombinationController(DiploidCombinationService diploidCombinationService) {
        this.diploidCombinationService = diploidCombinationService;
    }
    ...
    public Callable<ResponseEntity<Boolean>> deleteDiploidCombination(@RequestBody DiploidCombination diploidCombination) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(diploidCombinationService.deleteItem(diploidCombination), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<Boolean>> deleteDiploidCombination(@PathVariable String id) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(diploidCombinationService.deleteItem(id), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<DiploidCombination>> getModel() {
        try {
            ...
            return () -> new ResponseEntity<>(new DiploidCombination(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
