...
@RestController
@RequestMapping("/alleleAssignment")
@CrossOrigin
public class AlleleAssignmentController implements AlleleAssignmentApi {

    private final AlleleAssignmentService alleleAssignmentService;
    private static final Logger LOG = Logger.getLogger(AlleleAssignmentController.class);

    @Autowired
    public AlleleAssignmentController(AlleleAssignmentService alleleAssignmentService) {
        this.alleleAssignmentService = alleleAssignmentService;
    }
    ...
    public Callable<ResponseEntity<Boolean>> deleteAlleleAssignment(@RequestBody AlleleAssignment alleleAssignment) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(alleleAssignmentService.deleteItem(alleleAssignment), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<Boolean>> deleteAlleleAssignment(@PathVariable String id) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(alleleAssignmentService.deleteItem(id), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<AlleleAssignment>> getModel() {
        try {
            ...
            return () -> new ResponseEntity<>(new AlleleAssignment(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
