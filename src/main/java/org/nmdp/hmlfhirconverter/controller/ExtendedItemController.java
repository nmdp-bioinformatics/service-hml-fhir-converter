...
@RestController
@RequestMapping("/extendedItem")
@CrossOrigin
public class ExtendedItemController implements ExtendedItemApi {

    private final ExtendedItemService extendedItemService;
    private static final Logger LOG = Logger.getLogger(ExtendedItemController.class);

    @Autowired
    public ExtendedItemController(ExtendedItemService extendedItemService) {
    ...
    public Callable<ResponseEntity<Boolean>> deleteExtendedItem(@RequestBody ExtendedItem extendedItem) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(extendedItemService.deleteItem(extendedItem), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<Boolean>> deleteExtendedItem(@PathVariable String id) throws NotFoundException {
        try {
            return () -> new ResponseEntity<>(extendedItemService.deleteItem(id), HttpStatus.OK);
        } catch (Exception ex) {
    ...
    public Callable<ResponseEntity<ExtendedItem>> getModel() {
        try {
            ...
            return () -> new ResponseEntity<>(new ExtendedItem(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
